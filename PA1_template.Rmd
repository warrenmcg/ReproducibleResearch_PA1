...
## How will we impute the NA values?

I decided to fill on NA values based on the average steps for each interval for that weekday.
To do that, I needed to add a column for weekday. I do this by utilizing the POSIXlt class
...
## How will we preprocess the data to do our analysis? 
...
library(ggplot2)
qplot(as.POSIXct(totalStepsPerDay$date),totalStepsPerDay$totalSteps,geom="histogram",stat="identity",xlab="Date",ylab="Total Steps")
```
...
As mentioned above, I decided to impute missing values based on the average steps for an interval
on that day of the week. Here is how I would do that:

```{r imputeMissingValues}
updated.table <- activity.table
updated.table <- merge(updated.table, avgStepsPerWeekday, by.x=c("interval","weekday"))
```
